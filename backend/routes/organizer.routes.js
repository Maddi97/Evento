...
router.get("/organizerByCategory/:categoryId", limiter, (req, res) => {
  const categoryId = req.params.categoryId;
  Organizer.find({ "category._id": categoryId })
    .then((organizer) => res.send(organizer))
    .catch((error) => {
      console.error("Internal error organizer:", error);
      res.status(500).json({ message: "Internal error organizer:", error });
    });
});
...
router.post("/organizerByEventCategory", limiter, (req, res) => {
  ...
  Organizer.find({})
    .then((organizers) => {
      Event.find({ "category._id": { $eq: catId } })
        .then((events) => {
          organizers.map((o) => {
            events.map((ev) => {
              if (
                ev._organizerId.toString().trim() === o._id.toString().trim()
              ) {
                if (!orgs.includes(o)) {
                  orgs.push(o);
                }
